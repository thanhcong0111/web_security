...
Next, in the request, adding `'--` to the query. Then use up to CAST statement by inserting `' AND CAST(SELECT 1) as int)--`. We will recieve an error.

Switch to boolean type: `' AND 1=CAST(SELECT 1) as int)--` => Do not recieve error message, so we can follow this attack.

Change a little bit of adding query `' AND 1=CAST(SELECT username FROM users) as int)--`. The returned result shows that it's right cause we do not recieve error message.
...
And now, we recieve the password of administrator.
